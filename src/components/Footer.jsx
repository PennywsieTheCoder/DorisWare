...
export default function Footer({ storeName }) {
  ...
  useEffect(() => { supabase.from("store_social_links").select("platform, url").eq("is_active", true).order("sort_order").then(({ data }) => setSocialLinks((data ?? []).map((link) => ({ ...socialPlatforms[link.platform], href: link.url })).filter((link) => link.label))); }, []);
