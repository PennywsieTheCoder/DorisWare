...
import { CheckCircle2, Menu, Search, ShoppingCart, User, X, Sun, Moon } from "lucide-react";
...
export default function Header({ storeName }) {
  ...
  const { count, openCart, cartNotice, dismissCartNotice } = useCart();
  ...
  return (
    <>
      <header className={`sticky top-0 z-50 border-b border-stone-200/80 bg-white/95 backdrop-blur transition-shadow dark:border-stone-800 dark:bg-stone-900/95 ${hasScrolled ? "shadow-md shadow-stone-900/5 dark:shadow-black/20" : "shadow-none"}`}>
        <div className="grid min-h-14 w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-x-2 px-3 py-2 sm:px-6 lg:h-16 lg:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)] lg:gap-3 lg:py-0">
          <Link to="/" className="flex min-w-0 items-center gap-2.5">
            <img src={logoUrl} alt="DorisWare" className="h-9 w-9 shrink-0 rounded-full object-contain sm:h-10 sm:w-10" />
            <span className="hidden truncate font-serif text-xl font-semibold tracking-tight text-stone-900 dark:text-stone-100 lg:inline">{storeName}</span>
          </Link>
          ...
          <form onSubmit={handleSearch} className="col-start-2 row-start-1 flex min-w-0 items-center rounded-xl border border-stone-200 bg-stone-50 px-2.5 py-2 dark:border-stone-700 dark:bg-stone-800 lg:hidden">
            <Search size={16} className="shrink-0 text-stone-400" />
            <input aria-label="Search products by name" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search" className="ml-1.5 min-w-0 flex-1 bg-transparent text-sm text-stone-800 outline-none placeholder:text-stone-400 dark:text-stone-100" />
          </form>

          <div className="hidden items-center justify-end gap-1.5 lg:flex">
            <form onSubmit={handleSearch} className="flex items-center rounded-full border border-stone-200 bg-stone-50 px-3 py-1.5 dark:border-stone-700 dark:bg-stone-800"><Search size={16} className="shrink-0 text-stone-400" /><input aria-label="Search products by name" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} placeholder="Search products" className="ml-2 w-36 bg-transparent text-sm text-stone-800 outline-none placeholder:text-stone-400 dark:text-stone-100 xl:w-52" /></form>
          ...
          <div className="col-start-3 row-start-1 flex shrink-0 items-center justify-self-end gap-0.5 lg:hidden">
      ...
      {cartNotice && (
        <div role="status" className="fixed inset-x-3 bottom-4 z-[60] flex items-center gap-3 rounded-2xl border border-emerald-200 bg-white p-3 shadow-2xl dark:border-emerald-900/60 dark:bg-stone-900 lg:bottom-auto lg:left-auto lg:right-6 lg:top-20 lg:w-[26rem]">
          <CheckCircle2 size={21} className="shrink-0 text-emerald-600" />
          <p className="min-w-0 flex-1 truncate text-sm font-medium text-stone-800 dark:text-stone-100">{cartNotice.productName} added to your cart</p>
          <button type="button" onClick={openCart} className="shrink-0 text-sm font-semibold text-emerald-700 dark:text-emerald-400">View cart</button>
          <button type="button" onClick={dismissCartNotice} aria-label="Dismiss cart notification" className="shrink-0 rounded-full p-1 text-stone-400 hover:bg-stone-100 hover:text-stone-700 dark:hover:bg-stone-800 dark:hover:text-stone-200"><X size={16} /></button>
        </div>
      )}
