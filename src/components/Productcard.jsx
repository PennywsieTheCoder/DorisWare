    ...
    >
      ...
      <div className="flex h-full flex-col p-3">
        ...
        <div className="mt-1.5 flex items-center gap-1.5 text-xs">
          {reviewCount > 0 ? <span className="flex items-center gap-0.5 text-amber-400" aria-label={`${ratingAverage.toFixed(1)} out of 5 stars`}>{Array.from({ length: 5 }, (_, index) => <Star key={index} size={13} fill={index < Math.round(ratingAverage) ? "currentColor" : "none"} className={index < Math.round(ratingAverage) ? "text-amber-400" : "text-stone-300 dark:text-stone-600"} />)}</span> : <span className="font-semibold text-stone-500 dark:text-stone-400">New</span>}
        </div>
        ...
        <p className="mt-1.5 min-h-9 line-clamp-2 text-xs leading-relaxed text-stone-500 dark:text-stone-300">{description}</p>
        ...
        <div className="mt-3 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="flex shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-full bg-stone-100 px-3 py-1.5 text-xs font-semibold text-stone-900 transition-colors hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-stone-800 dark:text-stone-100 dark:hover:bg-stone-700"
          >
