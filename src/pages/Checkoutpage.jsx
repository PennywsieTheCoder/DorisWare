...
function savedAddressForm(user, address) {
  const nameParts = address?.recipient?.trim().split(/\s+/) ?? [];
  const profileNameParts = user?.name?.trim().split(/\s+/) ?? [];
  return {
    email: user?.email ?? "",
    firstName: nameParts[0] ?? profileNameParts[0] ?? "",
    lastName: nameParts.length ? nameParts.slice(1).join(" ") : profileNameParts.slice(1).join(" "),
    ...
    deliveryMethod: "dorisware_delivery",
    mobileProvider: "mtn",
    mobileNumber: address?.phone ?? user?.phone ?? "",
    saveAddress: !address,
    newsletter: false,
    terms: false,
  };
...
export default function CheckoutPage() {
  ...
  const initialAddress = user?.addresses?.find((address) => address.isDefault) ?? user?.addresses?.[0] ?? null;
  const [form, setForm] = useState(() => savedAddressForm(user, initialAddress));
