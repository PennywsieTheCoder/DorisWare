...
export default function CheckoutPage() {
  const navigate = useNavigate(); const { items, clearCart } = useCart(); const { user, updateProfile, addOrder, addAddress } = useAuth();
  ...
  function submit(event) {
    ...
    if (form.saveAddress && addAddress) {
    ...
    const newOrderObj = {
      ...
      status: "Processing",
      trackingNumber: `GH-EXP-${Math.floor(100000 + Math.random() * 900000)}`,
      estimatedDelivery: "3-5 Business Days",
      paymentMethod: form.payment === "mobile-money" ? "Mobile Money (MTN MoMo)" : "Debit or Credit Card",
