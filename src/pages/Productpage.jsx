    ...
    <div className="bg-stone-50 dark:bg-stone-950">
      <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="grid gap-8 lg:grid-cols-[1.05fr_.95fr] lg:items-start lg:gap-14">
          <section>
            <div className="flex min-h-[340px] items-center justify-center overflow-hidden rounded-[2rem] border border-stone-200 bg-stone-100 shadow-sm dark:border-stone-800 dark:bg-stone-900 sm:min-h-[500px]">
              <img src={product.images?.[imageIndex]} alt={product.name} className="h-full w-full object-cover" />
            </div>
            {product.images?.length > 1 && <div className="mt-4 flex gap-3">{product.images.map((image, index) => <button type="button" key={image} onClick={() => setImageIndex(index)} className={`flex h-16 w-16 items-center justify-center overflow-hidden rounded-xl border transition ${imageIndex === index ? "border-green-600 ring-2 ring-green-100 dark:ring-green-900" : "border-stone-200 dark:border-stone-700"}`} aria-label={`View product image ${index + 1}`}><img src={image} alt="" className="h-full w-full object-cover" /></button>)}</div>}
          </section>
