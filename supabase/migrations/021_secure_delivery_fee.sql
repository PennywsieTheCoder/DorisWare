-- Recalculate delivery from the shipping region instead of trusting a browser-supplied fee.
...
begin
  ...
  select delivery_zones.shipping_fee into v_shipping_fee
  from public.delivery_zones
  where delivery_zones.is_active = true
    and trim(p_shipping_address ->> 'region') = any(delivery_zones.regions)
  order by delivery_zones.sort_order
  limit 1;
  ...
  from public.promo_banners as promo where promo.is_active = true and (promo.ends_at is null or promo.ends_at > now())
  ...
  values (v_order_number, v_user_id, p_payment_method, 0, v_shipping_fee, 0, trim(p_contact_email), trim(p_contact_phone), p_shipping_address, nullif(trim(p_delivery_notes), ''))
